...
class AccountBloc extends Bloc<AccountEvent, AccountState> {
  ...
  AccountBloc({
    ...
    on<UpdateShipperMaxDistanceInAccountEvent>(
        _onUpdateShipperMaxDistanceInAccountEvent);
  }
  ...
  void _onUpdateShipperMaxDistanceInAccountEvent(
      UpdateShipperMaxDistanceInAccountEvent event,
      Emitter<AccountState> emit) async {
    emit(state.copyWith(status: AccountStatus.loading));
    try {
      final account = state.account;
      if (account == null || account.shipper == null) {
        throw Exception("Account or Shipper not found");
      }

      final updatedShipper =
          account.shipper!.copyWith(maxDistanceAllowance: event.maxDistance);
      final updatedAccount = account.copyWith(shipper: updatedShipper);

      // Save the updated max distance in shared preferences
      final prefs = await SharedPreferences.getInstance();
      await prefs.setDouble('maxDistance', event.maxDistance);

      emit(state.copyWith(
          status: AccountStatus.success, account: updatedAccount));
    } catch (error) {
      emit(state.copyWith(
          status: AccountStatus.failed, error: error.toString()));
    }
  }
